...
def calc_eval(xpos, ypos):
    reverse_list = []
    for dy, dx in check_list:
        nx, ny = xpos+dx, ypos+dy
        if field[ny][nx] == player_color:
            tmp = [(xpos, ypos)]
            while field[ny][nx] == player_color:
                tmp.append((nx, ny))
                nx += dx
                ny += dy

            if field[ny][nx] == cpu_color:
                while tmp:
                    reverse_list.append(tmp.pop())
            else:
                tmp.clear()
...
def cpu_turn():
    ...
    if not is_placeable_list:
        ...
        for x, y in is_placeable_list:
            evalu, reverse_cnt = calc_eval(x, y)
            if stone_count >= 59:
                if max_reverse_cnt < reverse_cnt:
                    max_reverse_cnt = reverse_cnt
                    xy = [x, y]
            else:
                if max_eval < evalu:
                    max_eval = evalu
